...
export const BudgetView: React.FC<Props> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Importante:
    // - En edición del mes actual: reemplaza (App ya hace upsert por categoría+mes).
    // - En edición de histórico "Fijo": crea/actualiza presupuesto de selectedMonth.
    onAdd({
        ...
        >
          ...
          {isEditMode && editingContext && editingContext.sourceMonth !== selectedMonth && (
            <p className="mb-4 text-[11px] font-bold text-slate-500 bg-slate-50 border border-slate-100 rounded-xl px-4 py-3">
              ...
              se creará/actualizará para <b>{selectedMonth}</b>.
            </p>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {budgetsWithSpent.map((b) => {
          ...
          return (
            <div
              key={b.id}
              className={`bg-white p-8 rounded-[2.5rem] border ${
                isOver ? 'border-rose-100' : 'border-slate-100'
              } shadow-sm relative overflow-hidden group transition-all hover:shadow-xl`}
            >
              <div className="flex justify-between items-start mb-6">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {isHistorical && (
                      <span
                        className="text-[8px] font-black bg-slate-100 text-slate-400 px-2 py-0.5 rounded-full uppercase tracking-tighter"
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-2 text-slate-300 hover:text-indigo-600 transition-all bg-slate-50 rounded-xl"
                    title={isHistorical ? 'Editar para este mes' : 'Editar presupuesto'}
                  >
                    <Pencil size={16} />
                  </button>
                  ...
                  {!isHistorical && (
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-2 text-slate-300 hover:text-rose-500 transition-all bg-slate-50 rounded-xl"
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  ...
                  <div
                    className={`px-2 py-1 rounded-lg text-[10px] font-black ${
                      isOver ? 'bg-rose-50 text-rose-600' : 'bg-slate-50 text-slate-400'
                    }`}
                  >
                ...
                <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                  ...
                  <span className="text-slate-400">
                    Restante: {b.currency === 'USD' ? '$' : 'Bs '}
                    {Math.max(0, limit - spent).toLocaleString()}
                  </span>
                </div>
              </div>
