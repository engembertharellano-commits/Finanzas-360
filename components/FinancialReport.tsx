...
export const FinancialReport: React.FC<FinancialReportProps> = ({
  ...
  // --- ESTILOS DE DISEÑO PREMIUM Y MARCA DE AGUA ---
  const premiumStyles = `
    @media print {
    ...
    <>
      ...
      <div className="report-bg-pattern min-h-screen py-8 no-print">
        ...
        <div className="print-container relative max-w-[210mm] mx-auto bg-white shadow-2xl overflow-hidden font-sans text-slate-800 border-t-8 border-blue-900">
          ...
          {/* HEADER */}
          <div className="bg-slate-900 text-white px-10 py-8 flex justify-between items-center relative z-10 print-break-avoid">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-blue-400 font-bold text-[10px] tracking-[0.3em] uppercase mt-1">Status: {selectedMonth}</p>
              </div>
            </div>
          ...
          <div className="p-10 space-y-10 relative z-10">
            ...
            {/* SECCIÓN 1: KPIs */}
            <div className="grid grid-cols-4 gap-4 print-break-avoid">
            ...
            {/* SECCIÓN 2: GRÁFICO HISTÓRICO REFINADO */}
            ...
            <div className="grid grid-cols-5 gap-10 print-break-avoid pt-4">
              <div className="col-span-3 space-y-6">
                ...
                {/* GRÁFICA DE BARRAS DE INVERSIÓN */}
                  ...
                  {assetBreakdown.slice(0, 5).map((inv, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center justify-end h-full z-10 w-full">
                        <div className="bg-slate-50 px-2 py-0.5 rounded shadow-sm border border-slate-200 text-[9px] font-black text-blue-700 mb-2">
                          {fPct(inv.roi)}
                        </div>
                        <div className="flex items-end gap-1 w-full justify-center flex-1 pb-1">
                          ...
                          <div className="w-4 bg-blue-700 shadow-sm" title="Valor Mercado" style={{height: `${Math.max((inv.current/max)*100, 2)}%`}} />
                        </div>
                ...
                {/* TABLA DE RESUMEN DE INVERSIONES */}
            ...
            {/* SECCIÓN 4: RESTAURADA (GASTOS MAYORES Y PROYECCIONES) */}
            <div className="grid grid-cols-2 gap-10 print-break-avoid pt-10 border-t border-slate-100">
              
              {/* Análisis de Gastos Mayores (Top 5) */}
              <div>
                <h2 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-6 flex items-center gap-2">
                  <BarChart2 size={16} className="text-blue-600"/> Resumen de Gastos Mayores
                </h2>
                <div className="space-y-3">
                  {spendingTable.length > 0 ? spendingTable.map(([cat, val], i) => (
                    <div key={i} className="relative bg-white p-3 rounded-xl border border-slate-100 overflow-hidden shadow-sm">
                      <div className="absolute left-0 top-0 bottom-0 bg-rose-100/50" style={{ width: `${(val/expenses)*100}%` }}></div>
              ...
              <div className="space-y-6">
                <h2 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-2 flex items-center gap-2">
                  <TrendingUp size={16} className="text-blue-600"/> Outlook Financiero (6 Meses)
                </h2>
                <div className="bg-slate-50 border border-slate-100 p-6 rounded-2xl space-y-3">
                  ...
                  <div className="flex justify-between items-center p-3 bg-emerald-50 border border-emerald-100 rounded-xl">
                    <span className="text-[10px] font-bold text-emerald-700 uppercase">Optimista (+20% Ingresos)</span>
                  ...
                  <div className="flex justify-between items-center p-3 bg-rose-50 border border-rose-100 rounded-xl">
                    <span className="text-[10px] font-bold text-rose-700 uppercase">Pesimista (+20% Gastos)</span>
            ...
            <div className="grid grid-cols-2 gap-10 pt-10 border-t border-slate-100 print-break-avoid">
              <AuditBox 
                title="Status de Liquidez" 
                value={fUSD(emergencyFund.current)} 
                desc={emergencyFund.current > (expenses * 3) ? "Excelente: Cubre más de 3 meses de gastos operativos." : "Alerta: Liquidez inferior al benchmark de seguridad (3 a 6 meses de gastos)."} 
              />
              <AuditBox 
                title="Eficiencia de Ahorro" 
                value={fPct(savingsRate).replace('+', '')} 
                desc={savingsRate > 20 ? "Margen operativo óptimo para expansión de capital y reinversión constante." : "Ratio ajustado. Se recomienda auditoría exhaustiva de gastos variables."} 
              />
            </div>

          </div>

          <div className="bg-slate-900 text-white/40 text-center py-4 text-[7px] uppercase tracking-[0.4em] font-black italic">
            Confidential Document • Finanza360 Intelligence Report • ISO-9001 Compliant
          </div>
        </div>
      </div>
    </>
...
const KPICard = ({ title, value, color, icon, extra }: any) => (
  <div className={`bg-white border-l-4 ${color} p-5 shadow-sm`}>
...
const AuditBox = ({ title, value, desc }: any) => (
  <div className="flex gap-4 items-start p-5 bg-white border border-slate-100 shadow-sm relative overflow-hidden">
